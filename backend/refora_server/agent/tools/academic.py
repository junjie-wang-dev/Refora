...
def get_related_academic_papers(executor: Any, args: dict[str, Any]) -> Any:
    academic = value(value(executor, "deps"), "academic")
    ...
    method = (
        "get_citing_papers" if relation == "citing" else "get_referenced_papers"
    )
    return call(
        value(academic, "graph"),
        method,
...
class AcademicTools(ToolGroup):
    ...
    handlers = {
        ...
        "get_related_academic_papers": get_related_academic_papers,
    ...
    descriptions = {
        ...
        "get_related_academic_papers": "Get a bounded page of papers related to one verified locator. relation=citing returns papers citing the target, referenced returns papers cited by it, and recommended returns Semantic Scholar recommendations.",
    ...
    schemas = {
        ...
        "get_related_academic_papers": object_schema({"relation": {"type": "string", "enum": ["citing", "referenced", "recommended"]}, **_GRAPH_PROPS}, ["relation", "paper"]),
