...
describe('preload IPC bridge', () => {
  ...
  it('forwards every request method to its shared IPC channel', async () => {
    const cases: InvocationCase[] = [
      ...
      { channel: IpcChannel.DocumentsReadPdfRange, args: ['doc-1', 0, 65536], invoke: (value) => value.documents.readPdfRange('doc-1', 0, 65536) },
      ...
      { channel: IpcChannel.ContextMenuShow, args: [[{ id: 'copy', label: 'Copy', icon: 'copy' }]], invoke: (value) => value.contextMenu.show([{ id: 'copy', label: 'Copy', icon: 'copy' }]) },
      { channel: IpcChannel.ClipboardReadFiles, args: [], invoke: (value) => value.clipboard.readFiles() },
      ...
      { channel: IpcChannel.AiChatHistoryPage, args: ['thread-1', { before: 'cursor', limit: 20 }], invoke: (value) => value.ai.chatHistoryPage('thread-1', { before: 'cursor', limit: 20 }) },
      { channel: IpcChannel.AiChatRunSnapshot, args: ['run-1', 12], invoke: (value) => value.ai.chatRunSnapshot('run-1', 12) },
      { channel: IpcChannel.AiMediaResolve, args: [{ source: { type: 'asset', assetId: 'asset-1' } }], invoke: (value) => value.ai.resolveMedia({ source: { type: 'asset', assetId: 'asset-1' } }) },
      { channel: IpcChannel.AiMediaTextPreview, args: ['media-1'], invoke: (value) => value.ai.mediaTextPreview('media-1') },
      { channel: IpcChannel.AiMediaOpen, args: ['media-1'], invoke: (value) => value.ai.openMedia('media-1') },
      { channel: IpcChannel.AiMediaReveal, args: ['media-1'], invoke: (value) => value.ai.revealMedia('media-1') },
      { channel: IpcChannel.AiMediaSave, args: ['media-1'], invoke: (value) => value.ai.saveMedia('media-1') },
      { channel: IpcChannel.AiMediaCopy, args: ['media-1'], invoke: (value) => value.ai.copyMedia('media-1') },
  ...
  it('subscribes every event method and forwards only the payload', () => {
    const eventCases: Array<[string, (cb: () => void) => () => void]> = [
      ...
      [IpcChannel.EventAiChatMedia, (cb) => api.events.onAiChatMedia(cb)],
