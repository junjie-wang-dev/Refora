...
class OcrMemoryTools(ToolGroup):
    ...
    descriptions = {
        "prepare_paper_ocr": "Run the local MinerU balanced OCR pipeline for a paper and prepare a reusable structured Markdown cache. Call this only after read_paper with source=ocr reports that no suitable OCR cache exists and OCR is necessary. Local OCR is allowed without additional approval.",
    }
    schemas = {
