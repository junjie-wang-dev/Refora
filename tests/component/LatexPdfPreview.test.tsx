...
it('delegates rendering and both navigation directions to the shared reader', async () => {
  ...
  const view = render(<LatexPdfPreview ref={handle} toolbarContainer={toolbarContainer} documentId="latex:project" data="cGRm" syncEnabled target={target} onLocateSource={locate} onDownload={download} />)
  ...
  expect(observed.props?.toolbarContainer).toBe(toolbarContainer)
