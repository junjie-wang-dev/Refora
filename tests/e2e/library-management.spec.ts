...
test('search, scoped sorting, multiple selection, compact controls and document deletion work together', async () => {
  ...
  try {
    ...
    await expect(compactRow.getByRole('checkbox')).toHaveCount(0)
    await compactRow.click({ modifiers: ['Meta'] })
    await expect(compactRow).toHaveAttribute('aria-selected', 'true')
